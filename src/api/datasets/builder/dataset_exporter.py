...
def export_dataset(builder: DatasetBuilder):
    ...
    for level_index, level in enumerate(builder.levels):
        level_export_path = os.path.join(
            builder.path, "Artifacts", "Level_%d.csv" % level_index
        )
    ...
    top_bottom_df.to_csv(
        os.path.join(builder.path, "Oracles", "Relations.csv"), index=False
    )
    ...
    for trace_id in builder.trace_matrices.keys():
        ...
        matrix_file_path = os.path.join(
            builder.path, "Oracles", "TracedMatrices", matrix_file_name
        )
