...
from api.technique.definitions.sampled.definition import SAMPLED_COMMAND_SYMBOL, SampledTechniqueDefinition
from api.technique.definitions.sampled.traces.technique import SAMPLED_TRACED_COMMAND_SYMBOL
from api.technique.definitions.transitive.definition import TRANSITIVE_COMMAND_SYMBOL, TransitiveTechniqueDefinition
...
class TestTechniqueHelper(SmartTest):
    ...
    transitive_component_a = [DIRECT_COMMAND_SYMBOL,
    ...
    transitive_upper_comp = "(%s (%s %s) (%s %s))" % (
        ...
        transitive_component_trace_type.value,
        "0", "1")
    transitive_component_b = [DIRECT_COMMAND_SYMBOL,
                              [transitive_algebraic_model.value,
                               transitive_component_trace_type.value],
                              ["1", "2"]]
    transitive_component_b_name = "(%s (%s %s) (%s %s))" % (
        DIRECT_COMMAND_SYMBOL,
        transitive_algebraic_model.value,
        transitive_component_trace_type.value, "1", "2")

    transitive_parameters = [transitive_aggregation_type.value, transitive_component_scaling_type.value]
    transitive_components = [transitive_component_a, transitive_component_b]
    transitive_technique_definition = [TRANSITIVE_COMMAND_SYMBOL, transitive_parameters, transitive_components]
    ...
    traced_direct_component_a = [DIRECT_COMMAND_SYMBOL,
                                 [transitive_algebraic_model.value,
                                  traced_component_type.value],
                                 ["0", "1"]]
    traced_direct_component_b = [DIRECT_COMMAND_SYMBOL,
                                 [transitive_algebraic_model.value,
                                  traced_component_type.value],
                                 ["1", "2"]]
    traced_components = [traced_direct_component_a, traced_direct_component_b]
    traced_parameters = [traced_aggregation_value.value, transitive_component_scaling_type.value]
    """
    Sampled Artifacts
    """
    ...
    sampled_artifacts_definition = [SAMPLED_COMMAND_SYMBOL, sampled_parameters,
                                    sampled_components]
    sampled_traces_definition = [SAMPLED_TRACED_COMMAND_SYMBOL, sampled_parameters,
                                 sampled_components]
    ...
    """
    combined_sampled_artifacts_components = [direct_definition, sampled_artifacts_definition]
    combined_sampled_traces_components = [direct_definition, sampled_traces_definition]

    direct_technique_name = "(. (VSM NT) (0 2))"
    transitive_technique_name = "(x (SUM GLOBAL) ((. (VSM NT) (0 1)) (. (VSM NT) (1 2))))"
    transitive_sampled_artifacts_technique_name = "(~ (SUM GLOBAL %f) ((. (VSM NT) (0 1)) (. (VSM NT) (1 2))))" % sample_percentage
    transitive_sampled_traces_technique_name = "($ (SUM GLOBAL %f) ((. (VSM NT) (0 1)) (. (VSM NT) (1 2))))" % sample_percentage
    combined_technique_name = "(o (%s) (%s %s))" % ("SUM", direct_technique_name, transitive_technique_name)
    combined_sampled_artifacts_technique_name = "(o (%s) (%s %s))" % (
        "SUM", direct_technique_name, transitive_sampled_artifacts_technique_name)
    combined_sampled_traces_technique_name = "(o (%s) (%s %s))" % (
        "SUM", direct_technique_name, transitive_sampled_traces_technique_name)
    ...
    def get_transitive_definition(self) -> TransitiveTechniqueDefinition:
        return TransitiveTechniqueDefinition(self.transitive_parameters, self.transitive_components)

    def get_traced_transitive_definition(self) -> TransitiveTechniqueDefinition:
        return TransitiveTechniqueDefinition(self.traced_parameters, self.traced_components)

    def get_combined_definition(self) -> CombinedTechniqueDefinition:
        return CombinedTechniqueDefinition(self.combined_parameters, self.combined_components)
    ...
    def get_combined_sampled_artifacts_definition(self) -> CombinedTechniqueDefinition:
        return CombinedTechniqueDefinition(self.combined_parameters, self.combined_sampled_artifacts_components)

    def assert_valid_fake_dataset_similarity_matrix(self, similarity_matrix: SimilarityMatrix):
