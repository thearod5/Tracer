...
from api.datasets.parsing.source_code_parser import get_relative_path_to_files_from, \
    parse_source_code
...
class TestSourceCodeParser(SmartTest):
    dataset = "derby"
    path_to_source_code = os.path.join(PATH_TO_ROOT, "SEProjects", dataset, "sourceCode")
    ...
    def test_relative_file_path_file_types(self):
        ...
        for sc_path in source_code_paths:
            ...
            self.assertTrue(extension in valid_extensions, "invalid extension: %s" % extension)
            self.assertTrue(os.path.isfile("/".join([self.path_to_source_code, sc_path])))
    ...
    def test_relative_file_path_on_sample_data(self):
        ...
        for file_path in expected_files:
            self.assertTrue(file_path in resulting_file_paths, "Expected to find %s in file graph_paths." % file_path)

    def test_parse_source_code(self):
        self.assertTrue(os.path.isdir(PATH_TO_SOURCE_CODE), 'source code path is broken')
        ...
        for expected_id in expected_ids:
            self.assertTrue(expected_id in list(level_3["id"]), "Could not find: %s" % expected_id)
