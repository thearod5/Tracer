...
class TestCombinedCalculationPipeline(TestTechniqueHelper):
    ...
    def test_combined_technique_calculator_with_custom_pipeline(self):
        counter_func, counter_dic = self.create_counter_func(
    ...
    def test_combined_technique_data(self):
        technique_data = CombinedTechniqueData(
            self.dataset, self.get_combined_definition()
        )
        self.assertEqual(
            self.combined_technique_name, technique_data.technique.get_name()
        )
    ...
    def test_turn_aggregated_values_into_matrix(self):
        reshaped = turn_aggregated_values_into_matrix(self.dataset, np.array([1, 2, 3]))
