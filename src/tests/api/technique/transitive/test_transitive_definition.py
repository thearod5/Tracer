...
class TestIntermediateDefinition(TestTechniqueHelper):
    def test_parse(self):
        definition = TransitiveTechniqueDefinition(self.transitive_technique_definition[1],
                                                   self.transitive_technique_definition[2])
        self.assertEqual(self.transitive_algebraic_model, definition.algebraic_model)
        self.assertEqual(self.transitive_aggregation_type, definition.transitive_aggregation)

        component_techniques: [DirectTechniqueDefinition] = definition.get_component_techniques()
        self.assertEqual(2, len(component_techniques))

        for component in component_techniques:
            self.assertEqual(self.transitive_algebraic_model, component.definition.algebraic_model)
            self.assertEqual(self.transitive_component_trace_type, component.definition.trace_type)

    def test_parse_with_wrong_parameters(self):
        self.assertRaises(Exception, lambda: TransitiveTechniqueDefinition(["VSM", "SUM"],
                                                                           self.transitive_components))

    def test_parse_with_wrong_components(self):
        self.assertRaises(Exception, lambda: TransitiveTechniqueDefinition(self.transitive_parameters,
                                                                           self.transitive_components[:1]))
    ...
    def test_parse_with_wrong_algebraic_model(self):
        ...
        self.assertRaises(Exception, lambda: TransitiveTechniqueDefinition(self.transitive_technique_definition[1],
                                                                           components))
