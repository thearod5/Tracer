...
from api.technique.variationpoints.aggregation.transitive_path_aggregation import (
    ...
    create_transitive_aggregation_training_data,
...
class TestIntermediateAggregation(SmartTest):
    upper = np.array([[1, 2, 3], [8, 9, 10]])  # (1, 10) R->D
    lower = np.array([[1, 2], [2, 3], [3, 4]])  # (10, 2) D-> C
    ...
    def test_aggregate_similarity_matrices_with_arithmetic_aggregator(self):
        result_matrix = aggregate_similarity_matrices_with_arithmetic_aggregator(
            self.similarity_matrices, AggregationMethod.MAX
        )
    ...
    def test_dot_product_with_pca_aggregation_with_sum(self):
        result_matrix = dot_product_with_aggregation(self.similarity_matrices, sum)
