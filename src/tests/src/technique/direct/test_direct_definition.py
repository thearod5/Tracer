...
class TestDirectDefinition(TestTechniqueHelper):
    def test_direct_technique_definition(self):
        definition = DirectTechniqueDefinition(
            self.direct_definition[1], self.direct_definition[2]
        )
    ...
    def test_direct_technique_definition_without_scaling(self):
        self.assertRaises(
            Exception, lambda: DirectTechniqueDefinition(["VSM"], ["0", "2"])
        )

    def test_direct_technique_definition_without_missing_artifact_levels(self):
        self.assertRaises(
            Exception, lambda: DirectTechniqueDefinition(["VSM", "GLOBAL", "NT"], ["0"])
        )
